    ...
    <div className='max-w-6xl mx-auto my-10'>
      <div className="w-full">
        <div className="relative w-full h-32 md:h-64 lg:h-72">
          <img src={singleRestaurant?.imageUrl || "Loading..."} alt="restaurant-image" className='object-cover w-full h-full rounded-lg shadow-lg' />
        </div>
            ...
            <div className='flex gap-2 my-2'>
              {
                singleRestaurant?.cuisines.map((cuisine, index)=>(
                  <Badge key={index} className="rounded-full">{cuisine}</Badge>
                ))
              }
            </div>
            <div className='flex md:flex-row flex-col gap-2 my-5'>
              <div className='flex items-center gap-2'>
                ...
                <h1 className='flex items-center gap-2 font-medium'>Delivery Time:{" "}
                  <span className='text-[#D19254]'>{singleRestaurant?.deliveryTime || "NA"} minutes</span>
                </h1>
              </div>
            </div>
          </div>
