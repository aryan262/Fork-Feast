...
import { Earth, Globe, MapPin, X } from 'lucide-react';
...
function SearchPage() {
  ...
  const {loading, searchedRestaurant, searchRestaurant, appliedFilter, setAppliedFilters} = useRestaurantStore();
  useEffect(()=>{
      ...
      <div className="flex flex-col md:flex-row justify-between gap-10">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <Input type="text" placeholder="Search by restaurant & cuisines" value={searchQuery} onChange={(event) => { setSearchQuery(event.target.value) }} />
          ...
          <div>
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:gap-2 my-3">
              <h1 className='font-medium text-lg'>({searchedRestaurant?.data.length}) results found</h1>
              <div className='flex flex-wrap gap-2 mb-4 md:mb-0'>
                {
                  appliedFilter.map((selectedFilter, index) => (
                    <div key={index} className='relative inline-flex items-center max-w-full'>
                      ...
                      <X onclick={()=>setAppliedFilters(selectedFilter)} size={16} className="absolute text text-[#D19254] right-1 hover:cursor-pointer" />
                    </div>
                  ))
                }
              </div>
                  ...
                  (
                      ...
                      <div className='flex mt-4 gap-2 flex-wrap'>
                        {
                          restaurant.cuisines.map((cuisine, index) => (
                            <Badge key={index} className="font-medium px-2 py-1 rounded-full shadow-sm">{cuisine}</Badge>
                          ))
                        }
                      </div>
                    </CardContent>
                    <CardFooter className="p-4 border-t dark:border-t-gray-700 border-t-gray-100 text-white flex justify-end">
                      <Link to={`/restaurant/${restaurant._id}`}>
                        <Button className="bg-orange hover:bg-orange font-semibold rounded-full py-2 px-4 shadow-md transition-colors duration-200">View Menus</Button>
                      </Link>
                    </CardFooter>
                  </Card>
                    ))
...
function NoResultFound ({ searchText }) {
  return (
